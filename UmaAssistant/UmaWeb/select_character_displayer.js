...
function createCharacterImageButton(jp_event_owner, tw_event_owner, img_name) {
    ...
    */
    char_img.src = "../UmaMisc/Image/Character/".concat(img_name);
    char_img.alt = "<jp_event_owner>".concat(jp_event_owner, "</jp_event_owner><tw_event_owner>").concat(tw_event_owner, "</tw_event_owner>");
...
function setSearchTextbox() {
    ...
    search_textbox.addEventListener("input", function () {
        var pattern = new RegExp(search_textbox.value + "|" + hiraganaToKatakana(search_textbox.value), "i");
        var character_icon_elements = document.querySelectorAll(".character_icon");
        for (var i = 0; i < character_icon_elements.length; i++) {
            var character_icon = character_icon_elements[i];
