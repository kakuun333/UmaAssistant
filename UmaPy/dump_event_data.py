...
process_choice_effect_data = util.read_json("../UmaData/new/process_choice_effect_data.json");
event_data_jp = util.read_json("../UmaData/new/event_data_jp.json") or DEAULT_EVENT_DATA_DICT;

########## functions ##########

### 加工 choice_effect
def process_choice_effect(choice_effect):
    ### 替換換行符 ###
    choice_effect = util.sub(choice_effect, "\n", "<br>");
    ...
    for wrong_word, correct_word in process_choice_effect_data["wrong_word"]["skill_name"].items():
        choice_effect = util.sub(choice_effect, wrong_word, correct_word);
...
def dump_event(event_data_type, rare, event_owner):
    ...
    for h4 in h4_list:
        ...
        for tr in tr_list:
            ...
            choice_effect = process_choice_effect(choice_effect);
            # print(choice_effect);
...
def dump_sapoka_event_data_jp():
    ...
    for tr in tr_list:
        ...
        # 寫入 json
        util.write_json("../UmaData/new/event_data_jp.json", event_data_jp);
...
########## 倒出角色事件資料 ##########
def dump_character_event_data_jp():
